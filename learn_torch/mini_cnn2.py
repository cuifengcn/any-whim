...
def make_imgs(num=10):
    if not os.path.isdir('./imgs'):
        os.mkdir('./imgs')
    num = num if num < len(words) else len(words)
    ran = random.sample(list(range(len(words))), num)
    _words = []
    for i in ran:
...
import time
def get_stamp_by_m(gap):
    tmin = time.localtime().tm_min
    tmin = tmin - (tmin % gap) # 每隔 n 分钟存储一次
    return time.strftime("%Y%m%d_%H", time.localtime()) + '{:02}'.format(tmin)

def load_file_by_end(end):
    r = []
    for i in os.listdir('.'):
        if i.endswith(end):
            r.append(i)
...
# 读取单字图片文件
def read_imginfos(file, class_types_list, max_len):
    ...
    if type(class_types_list) == str:
        class_types = list(class_types_list)
    elif type(class_types_list) == list and type(class_types_list[0]) == str:
        class_types = list(class_types_list)
    else:
        for i in os.listdir(file):
            if i.endswith('.jpg') or i.endswith('.png'):
    ...
    lens = len(class_types) / max_len
    if lens > 1:
        ceil = int(lens) + (1 if len(class_types) % max_len != 0 else 0)
    else:
        ceil = 1
    shape = {
        "shape_w": max_len,
        "shape_h": ceil,
        "shape_f": max_len + ceil,
    }
    class_types = {tp: (int(idx / max_len), (idx % max_len)) for idx, tp in enumerate(class_types)}
    types_class = {(int(idx / max_len), (idx % max_len)): tp for idx, tp in enumerate(class_types)}
    return imginfos, class_types, types_class, shape

# 生成 y_true 用于误差计算
def make_y_true(imginfo, class_types, shape):
    img = imginfo['img']
    x, y = class_types.get(imginfo['class'])
    clz = [0.]*shape["shape_f"]
    clz[x] = 1.
    clz[y + shape["shape_h"]] = 1.
    return torch.FloatTensor(clz)

def load_data(filepath, class_types_list=None, max_len=None):
    imginfos, class_types, types_class, shape = read_imginfos(filepath, class_types_list, max_len)
    train_data = []
    for imginfo in imginfos:
        train_data.append([torch.FloatTensor(imginfo['img']), make_y_true(imginfo, class_types, shape)])
    return train_data, {'class_types': class_types, 'shape': shape, 'types_class': types_class}
...
class MiniCNN(nn.Module):
    ...
    def __init__(self, clens, inchennel=3):
        super().__init__()
        self.oceil = clens
...
def train(train_data, class_types):
    ...
    BATCH_SIZE = 1000
    LR = 0.0001
    class_types = clz_info['class_types']
    clens = clz_info['shape']['shape_f']

    try:
        file = load_file_by_end('net.pkl')
        print(file)
    ...
    mloss = miniloss(clens).to(DEVICE)
    optimizer = torch.optim.Adam(net.parameters(), lr=LR)
    ...
    for epoch in range(epoch, epoch+EPOCH):
        ...
        for step, (b_x, b_y) in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
        state = {'net':net.state_dict(), 'optimizer':optimizer, 'epoch':epoch+1, 'clz_info':clz_info}
        torch.save(state, get_stamp_by_m(5) + '_net.pkl')
        print('save.')
    print('end.')
...
def test(filename, state):
    ...
    clz_info = state['clz_info']
    ceil = clz_info['shape']['shape_h']
    types_class = clz_info['types_class']
    ...
    v_c = v[0:ceil]
    v_n = v[ceil:]
    v_x = v_c.index(max(v_c))
    v_y = v_n.index(max(v_n))
    if (v_x, v_y) in types_class:
        r = types_class[(v_x, v_y)]
    else:
        r = None
    img = cv2.imdecode(np.fromfile(filename, dtype=np.uint8), 1)
    def get_name(file):
...
while True:
    make_imgs()
    train_data, clz_info = load_data('./imgs', words, max_len=64)
    train(train_data, clz_info)
    print('loading model.')
    state = load_state(load_file_by_end('net.pkl'))
    print('loading model. ok.')
    print(state['clz_info']['class_types'])
    import os
    p = 0
    q = 0
    for idx, i in enumerate(os.listdir('imgs')):
        v = test('./imgs/{}'.format(i), state)
        if v:
            p += 1
        else:
            q += 1
