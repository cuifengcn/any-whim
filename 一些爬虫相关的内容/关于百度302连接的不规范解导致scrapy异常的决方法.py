...
# 百度在处理 302 的时候返回的信息并不正常，以下的测试代码(需打开fiddler的8888端口代理执行)执行后也能看出
# 百度直接返回的 Loc 后面并没有冒号，但是经过 fiddler 代理的却有，这是因为 fiddler 自动对该 302 跳转返回的信息进行了修正
# 但是如果直接 scrapy 请求就会出现异常。所以你需要通过一些方式进行处理才能让 scrapy 能够正常执行。
