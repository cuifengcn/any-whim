...
# 由于一些判决是直接在“高级法院”处理，而“地方法院”没有相关记录，所以这部分的数据可能会丢失，丢失比例预估是 5%。所以基本忽略即可。
def get_all_docid_by_oneday(day):
    ...
    for filter_info in filter_infos:
        ...
        # 下面这块代码仅作为测试用，这样可以只测试一条最细过滤条件过滤后的某一天的所有 docid，如果有需要请下面这块代码！！！！！！
...
if __name__ == '__main__':
    ...
    # 如果上面两条测试都跑失败了，估计加密已经过期了，
    # 如果是跑着跑着出现异常，请考虑以下是否是被封IP，是否是请求太快了，慢一点，给网站一条活路
    # 都写到这份上了，再看不懂，我就开始怀疑你的智商了，代码不明白，请仔细看代码，仔细看注释，问百度，问你爸你妈都可以
    # 如果你在看不懂的基础上问再我日期怎么迭代？我TM就干死你！