...
def mk_url_headers_body(songkey):
    url = 'http://music.y444.cn/api/v1/jx/v4/'
    headers = { "User-Agent": "Chrome/84.0.4147.125 Safari/537.36" }
    body = { "params": enc_post_params(json.dumps({"id":songkey,"q":"128","src":"wyy","play":True})), # q代表质量固定即可，这里的src就是网易云固定即可，play固定即可，所以这里你只需要音乐id的参数即可
...
songkey = 414817
url, headers, body = mk_url_headers_body(songkey)
