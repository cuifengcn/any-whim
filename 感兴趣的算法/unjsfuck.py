...
def plus(ls):
    ...
    # def f(i): return get_fit(i)[2] in (_str, )
    # def w(a,b): return _plus(get_fit(a), get_fit(b))
    # v = l

    # import itertools
    # import functools
    # n = []
    # for i in itertools.groupby(v, f):
    #     if i[0]:
    #         i = list(i[1])[::-1]
    #         if i:
    #             n.append(functools.reduce(w, i)[1])
    #     else:
    #         n.extend(list(i[1]))
    # print(n[::-1])
    # l = n
    # 这里进行一定的优化，优先将所有字符串类型的先合并
    # 因为 + 运算中只要含有字符串和列表最终都会生成字符串
    # 大部分的处理最终都是字符串
...
def some(s, dlevel=10):
    ...
    v = s
    def e(g): return repa(g.group(0))
    def f(g): return plus(cuter(repb(g.group(0)[1:-1]).replace(unique,unit)))
    def h(g): return uleft + g.group(1) + uright
    def j(g): return '[{}]'.format(f(g))
    for i in range(dlevel):
        v = re.sub(expc, e, v.replace(unit,unique))
        v = re.sub(expd, h, v.replace(unit,unique))
        v = re.sub(expa, f, v)
    ...
    return re.sub(r"('[^']+')\(", lambda i:"({})(".format(i.group(1)), v)
