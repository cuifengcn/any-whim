...
def plus(ls):
    ...
    e = [
        ...
        [r"^\['([^']+)'\]$",        None,       _str],
        [r'^\[([^\[\]]*)\]$',       None,       _list],
        [r"^\+?\[\]\['([^'\[\]]+)'\]$","_getfunc", _getfunc], # 这里存在查找函数可能，若找不到才会是 undefined
        ...
        for i in e:
            ...
            if v:
                if   i[1] is None:      i[1] = v[0]
                    ...
                    f2string = _get_func(v[0])
                    if f2string:
                        i[1] = f2string
                    else:
                        i[1], i[2] = v[0], _undefined
                return i
        return ['', '', _unfind]
    ...
    def _plus(a, b):
        ...
        bk, bv, bt = b
        types = [at, bt]
        if _list in types or _str in types or _getfunc in types:
            rt = _str
...
def some(s):
    ...
    def f(g): return plus(cuter(g.group(0)[1:-1].replace(unique,unit).replace(uleft,'[').replace(uright,']')))
    def h(g): return uleft + g.group(1) + uright
    def j(g): return '[{}]'.format(f(g))
    v = re.sub(expc, h, s.replace(unit,unique))
    v = re.sub(expd, h, v)
    v = re.sub(expa, f, v).replace(unique,unit)
    v = re.sub(expc, h, v.replace(unit,unique))
    v = re.sub(expb, j, v).replace(unique,unit)
    v = v.replace(uleft,'[').replace(uright,']')
    print(v)
    print()
